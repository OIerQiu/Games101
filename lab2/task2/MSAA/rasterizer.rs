...
impl Rasterizer {
    ...
    pub fn rasterize_triangle(&mut self, t: &Triangle) {
        ...
        //((self.height - 1 - y as u64) * self.width + x as u64) as usize
        for i in xmin*2..xmax*2+2 {
            for j in ymin*2..ymax*2+2 {
                let x = i as f64 / 2.0;
                let y = j as f64 / 2.0;
                if inside_triangle(x, y, &t.v) {
                    ...
                    if (nz != 0.0)		
                    {
                        z = t.v[0].z - (nx * (x - t.v[0].x) + ny * (y - t.v[0].y)) / nz;
                    }
                    let index = (((self.height as f64 - 1.0 - y) * 2.0) as u64 * 2 * self.width + i as u64) as usize;
                    if z < self.depth_buf[index] {
                        self.depth_buf[index] = z;
                        self.frame_buf[index].x = t.get_color().x;
                        self.frame_buf[index].y = t.get_color().y;
                        self.frame_buf[index].z = t.get_color().z;
                    }
                }
            }
        }
...
fn inside_triangle(x: f64, y: f64, v: &[Vector3<f64>; 3]) -> bool {
    ...
    for i in 0..3 {
        let a:f64 = x+0.5 - v[i].x;
        let b:f64 = y+0.5 - v[i].y;
        let c:f64 = v[(i+1) % 3].x - v[i].x;
        let d:f64 = v[(i+1) % 3].y - v[i].y;
        f[i] = a * d - b * c;
    }
    if (f[0] >= 0.0 && f[1] >=0.0 && f[2]>=0.0) || (f[0]<=0.0 && f[1]<=0.0 && f[2]<=0.0) {
        return true;
    }
    false
}
